...
while (True):
	...
	while not(gol):
		...
		if (not(y_bola+raio <= y1_trave and x_bola >= x1_trave2)) and (not(y_bola+raio <= y1_trave and x_bola <= x2_trave1)):
			if y_bola+raio < chao:
				if vely_bola < 12:
					vely_bola += 0.4
		...
		if colisao_cabeca <= 80:
			if x_bola-raio >= x_cabeca+raio_cabeca:
				if (x_cabeca-raio_cabeca)<=(x_bola-raio+velx_bola)<=(x_cabeca+raio_cabeca):
		...
		if colisao_cabeca2 <= 80:
			...
			if x_bola+raio <= x_cabeca2-raio_cabeca:
				if (x_cabeca2-raio_cabeca)<=(x_bola+raio+velx_bola)<=(x_cabeca2+raio_cabeca):
		...
		if not(2*raio >= colisao_cabeca and 2*raio >= colisao_pe):
			if 2*raio >= colisao_pe: ### COLISÃO COM O PÉ ###
				...
				if y_bola+raio+vely_bola > chao:
					vely_bola = chao-(y_bola+raio)
			if not(chute):
				if 2*(raio_cabeca-8) >= colisao_cabeca: ### COLISAO COM A CABEÇA ###
					if (y_cabeca <= y_bola and x_cabeca >= x_bola) or (y_cabeca >= y_bola and x_cabeca >= x_bola):
		...
		if not(2*raio >= colisao_cabeca2 and 2*raio >= colisao_pe2):
			if 2*raio >= colisao_pe2: ### COLISÃO COM O PÉ ###
				...
				if y_bola+raio+vely_bola > chao:
					vely_bola = chao-(y_bola+raio)
			if not(chute2):
				if 2*(raio_cabeca-8) >= colisao_cabeca2: ### COLISAO COM A CABEÇA ###
					if (y_cabeca2 <= y_bola and x_cabeca2 >= x_bola) or (y_cabeca2 >= y_bola and x_cabeca2 >= x_bola):
		...
		if not(chute):
			...
			if cont_chute//1 == 0:
				pe.move(0, 8)
				boneco.undraw()
				boneco = Image(Point(boneco.getAnchor().getX() , boneco.getAnchor().getY()), 'Imagens/LeftChar_kick1.gif')
				boneco.draw(tela)
			if cont_chute//8 == 1:
				pe.move(2,-2)
				boneco.undraw()
				boneco = Image(Point(boneco.getAnchor().getX() , boneco.getAnchor().getY()), 'Imagens/LeftChar_kick2.gif')
				boneco.draw(tela)
			cont_chute += 1
			if cont_chute == 15:
				boneco.undraw()
				boneco = Image(Point(boneco.getAnchor().getX() , boneco.getAnchor().getY()), 'Imagens/LeftChar.gif')
				boneco.draw(tela)
				cont_chute = 0
				pe.move(-14, 6)
				chute = False
		...
		if not(chute2):
			...
			if cont_chute2//1 == 0:
				pe2.move(0, 8)
				boneco2.undraw()
				boneco2 = Image(Point(boneco2.getAnchor().getX() , boneco2.getAnchor().getY()), 'Imagens/RightChar_kick1.gif')
				boneco2.draw(tela)
			if cont_chute2//8 == 1:
				pe2.move(-2,-2)
				boneco2.undraw()
				boneco2 = Image(Point(boneco2.getAnchor().getX() , boneco2.getAnchor().getY()), 'Imagens/RightChar_kick2.gif')
				boneco2.draw(tela)
			cont_chute2 += 1
			if cont_chute2 == 15:
				boneco2.undraw()
				boneco2 = Image(Point(boneco2.getAnchor().getX() , boneco2.getAnchor().getY()), 'Imagens/RightChar.gif')
				boneco2.draw(tela)
				cont_chute2 = 0
				pe2.move(14, 6)
				chute2 = False
